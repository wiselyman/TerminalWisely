...
async def run_investigator(
    ...
    *,
    ...
    child_meta: dict[str, Any] = {
        ...
        "origin": "investigator",
        "engineer_mode": mode,
        # Isolation over compression: child never inherits parent transcript.
    ...
    # Hard isolation: child SessionLog starts empty — never copy parent.messages.
    assert not child.messages, "investigator must not inherit parent transcript"
    ...
    if not summary and child.error:
        summary = child.error
    # Structured return only — do not dump child trajectory into parent context.
    if len(summary) > 2500:
        summary = summary[:2499] + "…"
