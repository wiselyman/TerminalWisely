...
class SessionApprovalCache:
    ...
    def invalidate_session_id(self, session_id: str) -> int:
        """Drop all cache buckets for an SSH/UI session_id (any host fingerprint)."""
        sid = (session_id or "").strip()
        if not sid:
            return 0
        dead = [k for k in self._entries if k.split("|", 1)[0] == sid]
        for k in dead:
...
# Last chat-start identity per session_id — used to invalidate on host change.
_LAST_IDENTITY_BY_SESSION: dict[str, TargetSessionIdentity] = {}


def sync_identity_and_invalidate(identity: TargetSessionIdentity) -> bool:
    """If target identity changed for this session_id, clear prior session allows.

    Returns True when a previous identity existed and differed (cache invalidated).
    """
    sid = (identity.session_id or "").strip()
    if not sid:
        return False
    prev = _LAST_IDENTITY_BY_SESSION.get(sid)
    _LAST_IDENTITY_BY_SESSION[sid] = identity
    if prev is None:
        return False
    if identity_fingerprint(prev) == identity_fingerprint(identity):
        return False
    GLOBAL_APPROVAL_CACHE.invalidate_session(prev)
    GLOBAL_APPROVAL_CACHE.invalidate_session_id(sid)
    return True


def clear_identity_tracker() -> None:
    _LAST_IDENTITY_BY_SESSION.clear()
