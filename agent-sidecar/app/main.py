...
from app.harness.approval_cache import sync_identity_and_invalidate
...
def _compose_user_message(
    ...
    *,
    force_current_turn: bool = False,
) -> str | list[dict[str, Any]]:
    return compose_user_content(
        message, attachments, force_current_turn=force_current_turn
    )
...
async def chat_start(body: ChatStartRequest, _: AuthDep) -> ChatStartResponse:
    ...
    sync_identity_and_invalidate(identity)
...
async def chat_continue(body: ChatContinueRequest, _: AuthDep) -> ChatStartResponse:
    ...
    run.interaction_mode = interaction
    user_message = _compose_user_message(
        body.message, body.attachments, force_current_turn=True
    )
    if body.attachments:
        run.append_event(
            "context_attached",
...
async def skills_list(
    _: AuthDep, engineer_mode: str | None = None
) -> dict[str, Any]:
    """User-authored skills on this machine (panel UI), scoped by engineer_mode."""
    from app.skills.loader import list_user_skills_catalog

    mode = (engineer_mode or "linux").strip().lower()
    if mode not in {"linux", "k8s"}:
        mode = "linux"
    catalog = list_user_skills_catalog(engineer_mode=mode)
    return catalog


@app.get("/v1/memory/meta")
async def memory_meta(
    _: AuthDep, engineer_mode: str | None = None
) -> dict[str, Any]:
    """Memory paths + counts for panel UI (read-only), scoped by engineer_mode."""
    from app.memory.catalog import memory_meta_catalog

    mode = (engineer_mode or "linux").strip().lower()
    if mode not in {"linux", "k8s"}:
        mode = "linux"
    return memory_meta_catalog(engineer_mode=mode)


@app.get("/v1/media/{media_id}")
async def media_get(media_id: str, _: AuthDep) -> dict[str, Any]:
    """Return a cached chat image as a data URL (WebView-safe)."""
    from app.media_cache import load_media_data_url

    try:
        return load_media_data_url(media_id)
...
@app.post("/v1/media/fetch")
async def media_fetch(body: dict[str, Any], _: AuthDep) -> dict[str, Any]:
    """Fetch a public image URL into the media cache and return a data URL."""
    from app.media_cache import load_media_data_url
    ...
    try:
        payload = await provider.web_fetch(url)
    ...
    if not payload.get("ok") or payload.get("kind") != "image":
        raise HTTPException(
            status_code=400,
            detail=str(payload.get("error") or "not an image"),
        )
    media_id = str(payload.get("media_id") or "")
    try:
        loaded = load_media_data_url(media_id)
...
async def models_list(body: ModelListRequest, _: AuthDep) -> ModelListResponse:
    ...
    try:
        ...
        if configured:
            ...
            auto_corrected = resolved != configured and resolved in models
        return ModelListResponse(
            models=models,
            error=None,
