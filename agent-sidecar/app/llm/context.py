...
def truncate_tool_payload(payload: dict[str, Any], *, limit: int = _MAX_TOOL_CONTENT_CHARS) -> dict[str, Any]:
    out = dict(payload)
    # Image tool results / HTML pages with extracted images must keep media_id
    # for the UI — never collapse into the generic "ok/stdout only" slim object.
    if out.get("media_id") and (
        out.get("kind") == "image" or isinstance(out.get("images"), list)
    ):
        slim_img: dict[str, Any] = {
            "ok": out.get("ok", True),
            "kind": out.get("kind"),
            "url": out.get("url"),
            "status": out.get("status"),
            "media_id": out.get("media_id"),
            "cached_path": out.get("cached_path"),
            "content_type": out.get("content_type"),
            "bytes": out.get("bytes"),
            "markdown": out.get("markdown"),
            "text": _clip(str(out.get("text") or ""), 2000) if out.get("kind") != "image" else "",
            "image_candidates": out.get("image_candidates"),
            "images": out.get("images"),
            "_untrusted": out.get("_untrusted", True),
            "_note": _clip(str(out.get("_note") or ""), 240),
        }
        return {k: v for k, v in slim_img.items() if v is not None}
    if out.get("kind") == "image" and out.get("media_id"):
        return {
            "ok": out.get("ok", True),
            "kind": "image",
            "url": out.get("url"),
    ...
    # Never forward huge data_url blobs into model context.
...
def compact_messages_for_model(
    ...
    *,
    ...
    tools_overhead_tokens: int = 2_000,
) -> list[dict[str, Any]]:
