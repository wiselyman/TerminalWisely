...
def estimate_tools_schema_tokens(tools: list[dict[str, Any]] | None) -> int:
    """Estimate tokens for the tools array sent with chat/completions."""
    if not tools:
        return 0
    # Floor covers schema + reply headroom; measure actual JSON when larger.
    measured = estimate_tokens(json.dumps(tools, ensure_ascii=False))
    return max(2_000, measured)
