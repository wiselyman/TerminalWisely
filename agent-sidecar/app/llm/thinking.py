...
def strip_trailing_dangling_heading(text: str | None) -> str:
    """Remove restart headings glued or dangling after a substantial answer."""
    raw = (text or "").rstrip()
    if not raw:
        return ""

    def _title_seen_in(body: str, title: str) -> bool:
        if not title:
            return False
        for ln in body.splitlines():
            m = _MD_HEADING_TITLE_RE.match(ln.strip())
    ...
    while True:
        ...
        if not last or not _MD_HEADING_LINE_RE.match(last):
            break
        title_m = _MD_HEADING_TITLE_RE.match(last)
        title = (title_m.group(1).strip() if title_m else "").strip()
        earlier = "\n".join(lines[:-1])
        if not _title_seen_in(earlier, title) and len(earlier.strip()) < 80:
            break
        lines.pop()
        raw = "\n".join(lines).rstrip()
    ...
    lines = raw.splitlines()
    if lines:
        glued = _GLUED_HEADING_LINE_RE.match(lines[-1])
        if glued:
            before, heading = glued.group(1), glued.group(2)
            title_m = _MD_HEADING_TITLE_RE.match(heading.strip())
            title = (title_m.group(1).strip() if title_m else "").strip()
            earlier = "\n".join(lines[:-1])
            if _title_seen_in(earlier, title) or len(re.sub(r"\s+", "", before)) >= 40:
...
def looks_like_truncated_answer(text: str | None) -> bool:
    ...
    if (
        len(headings) >= 2
        and headings[-1].strip() == headings[0].strip()
...
class StreamContentFilter:
    ...
    def finalize(self) -> str:
        ...
        # sanitize / extract_first can collapse a long streamed answer after a
        # false repetition hit — prefer the longer already-shown text.
        if cleaned and visible:
            if len(visible) > len(cleaned) * 1.15 and len(visible) - len(cleaned) > 120:
                return visible
        if cleaned:
            return cleaned
        if self.loop_detected:
