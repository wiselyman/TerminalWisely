...
_SHELL_C_BINS = frozenset({"bash", "sh", "zsh", "dash", "ksh"})


def peel_shell_c_script(argv: list[str]) -> str | None:
    """If argv is ``bash|sh -c 'script'`` (optional -l/-e), return the script body."""
    if not argv:
        return None
    name = argv[0].rsplit("/", 1)[-1].lower()
    if name not in _SHELL_C_BINS:
        return None
    i = 1
    while i < len(argv):
        tok = argv[i]
...
def extract_do_done_bodies(command: str) -> list[str]:
    """Extract ``do … done`` bodies from for/while/until compounds (quote-aware)."""
    raw = command or ""
    bodies: list[str] = []
    i = 0
    n = len(raw)
    ...
    def _at_word(word: str, pos: int) -> bool:
        if not raw.startswith(word, pos):
            return False
        end = pos + len(word)
        if end < n and (raw[end].isalnum() or raw[end] == "_"):
            return False
        if pos > 0 and (raw[pos - 1].isalnum() or raw[pos - 1] == "_"):
            return False
        return True
    ...
    while i < n:
        ...
        if _at_word("do", i):
            ...
            depth = 1
            j = start
            quote_inner: str | None = None
            while j < n:
                cj = raw[j]
                if quote_inner:
                    if cj == quote_inner and raw[j - 1] != "\\":
                        quote_inner = None
                    j += 1
                    continue
                if cj in "'\"":
...
def has_file_redirect(argv: list[str], raw: str) -> bool:
    ...
    # Require that after '>' we are NOT writing to /dev/null (optional spaces).
