...
def classify_command(command: str, bundle: PolicyBundle | None = None) -> tuple[RiskLevel, set[str]]:
    ...
    worst = RiskLevel.R0
    for leaf in expand_policy_leaves(cmd):
...
def expand_policy_leaves(command: str, *, _depth: int = 0) -> list[Leaf]:
    """Expand ``bash -c`` / loop bodies so policy sees real binaries, not wrappers."""
    if _depth > 8:
        return iter_leaves(command)
    out: list[Leaf] = []
    for leaf in iter_leaves(command):
        script = peel_shell_c_script(leaf.argv) if leaf.argv else None
        if script:
            out.extend(expand_policy_leaves(script, _depth=_depth + 1))
            continue
        if leaf.binary in {"for", "while", "until"}:
            bodies = extract_do_done_bodies(leaf.raw)
            if bodies:
                for body in bodies:
                    out.extend(expand_policy_leaves(body, _depth=_depth + 1))
                continue
        out.append(leaf)
    return out
...
def command_has_caps(command: str, wanted: set[str], bundle: PolicyBundle | None = None) -> bool:
    bundle = bundle or load_policy()
    for leaf in expand_policy_leaves(command):
        caps = resolve_leaf(leaf, bundle)
        if caps & wanted:
            return True
    return False
