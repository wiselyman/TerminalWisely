...
CURRENT_TURN_PREFIX = (
    ...
    "Prior unfinished host work (including long downloads already in progress) "
    "is background only — do not restart, re-clone, re-download, or kill those "
    "jobs unless this message explicitly asks. If this message only asks status/"
    "progress, use read-only checks and answer; do not mutate. "
    "If the user corrected you (e.g. 「不是让你…是让你…」 / "
    "\"I didn't ask you to X, do Y\"), abandon X immediately and do Y only.\n\n"
)

# User is switching away from prior unfinished work — always pin CURRENT_TURN.
_TASK_REDIRECT_RE = re.compile(
    r"(?:"
    r"我不是让你|不是让你|别(?:再)?(?:去)?|不要(?:再)?(?:去)?|"
    r"别测|不要测|别跑|不要跑|我说的是|我要你|请改成|改成|"
    r"i\s*didn'?t\s+ask|don'?t\s+(?:do|run|test)|stop\s+(?:doing|testing)|"
    r"instead\b|rather\s+than|not\s+what\s+i\s+(?:asked|meant)"
    r")",
    re.IGNORECASE,
)


def looks_like_task_redirect(message: str) -> bool:
    """True when the user is correcting / replacing the prior task."""
    text = (message or "").strip()
    if not text:
        return False
    return _TASK_REDIRECT_RE.search(text) is not None
...
def compose_user_content(
    ...
    *,
    ...
    Pin [CURRENT_TURN] when attachments distract, the user redirects the task, or
    force_current_turn (SessionLog resume / continue). Do not pin every plain
    fresh-chat message — that confuses small local models into fake JSON instead
    of tool_calls (main-branch rationale).
    """
    blocks = attachments_to_user_blocks(list(attachments or []))
    text = (message or "").strip()
    joined = "\n\n".join(blocks)
    pin_current = (
        force_current_turn or bool(joined) or looks_like_task_redirect(text)
    )
    if text and joined:
        full_text = f"{CURRENT_TURN_PREFIX}{text}\n\n{joined}"
    elif text and pin_current:
        full_text = f"{CURRENT_TURN_PREFIX}{text}"
    elif text:
        full_text = text
    elif joined:
