...
def test_bash_c_curl_allows_like_bare_curl():
    eng = PolicyEngine()
    d = eng.decide(
        "bash -c 'curl -s http://127.0.0.1:11434/api/chat'",
...
def test_bash_c_loop_curl_devnull_allows():
    eng = PolicyEngine()
    d = eng.decide(
        "bash -c 'for s in 1; do curl -s http://127.0.0.1:1 > /dev/null; done'",
        security_mode=SecurityMode.SAFE,
    )
    assert d.action == PolicyAction.ALLOW, (d.risk, d.reason)
    assert d.risk == RiskLevel.R0
...
def test_for_loop_rm_still_requires_approval():
    eng = PolicyEngine()
    d = eng.decide("for s in 1 2; do rm -rf /tmp/x; done", security_mode=SecurityMode.SAFE)
    assert d.action == PolicyAction.REQUIRE_APPROVAL
    assert d.risk.value >= "R2"


def test_space_before_devnull_not_write_upgrade():
    from app.policy.parse import has_file_redirect

    assert not has_file_redirect(["curl", "-s", "http://x"], "curl -s http://x > /dev/null")
    assert has_file_redirect(["curl", "-s", "http://x"], "curl -s http://x > /tmp/out.json")
