...
def test_truncate_tool_payload_keeps_image_media_id() -> None:
    payload = {
        "ok": True,
        "kind": "image",
        "media_id": "a" * 64,
        "markdown": f"![](media:{'a' * 64})",
        "url": "https://example.com/a.png",
        "cached_path": "/tmp/" + ("p" * 200),
    ...
    assert slim["media_id"] == "a" * 64
    assert "data_url" not in slim
    assert "markdown" in slim
