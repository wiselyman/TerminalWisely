...
def test_resolve_keeps_alias_when_catalog_id_is_filesystem_path() -> None:
    """SGLang may list HF path as id; do not overwrite a readable configured alias."""
    catalog = parse_openai_models_catalog(
        {
            "data": [
                {
                    "id": "/root/.cache/huggingface/hub/models--RadixArk--Qwen3.8-Flash-Next-NVFP4/snapshots/abc",
                    "root": "/root/.cache/huggingface/hub/models--RadixArk--Qwen3.8-Flash-Next-NVFP4/snapshots/abc",
                }
            ]
        }
    )
    assert resolve_served_model_id("Qwen3.8-spark", catalog) == "Qwen3.8-spark"
    assert resolve_served_model_id(catalog[0]["id"], catalog) == catalog[0]["id"]
