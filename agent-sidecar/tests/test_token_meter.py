...
def test_estimate_tools_schema_tokens_floor() -> None:
    assert estimate_tools_schema_tokens(None) == 0
    assert estimate_tools_schema_tokens([]) == 0
    small = [{"type": "function", "function": {"name": "a", "parameters": {}}}]
    assert estimate_tools_schema_tokens(small) == 2_000
    huge = [
        {
            "type": "function",
            "function": {
    ...
    assert estimate_tools_schema_tokens(huge) > 2_000
