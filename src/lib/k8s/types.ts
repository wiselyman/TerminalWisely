...
export interface K8sClusterTarget {
  ...
  /** SSH kubectl: run via passwordless sudo when kubeconfig is root-only (k3s). */
...
export interface K8sResourceRow {
  ...
  disk_requests?: string | null;
  disk_limits?: string | null;
...
export interface HelmRepoRow {
  name: string;
  url: string;
}

export interface K8sCrdCatalogEntry {
  group: string;
  kind: string;
  plural: string;
  name: string;
...
export interface K8sClusterSummary {
  ...
  metrics?: K8sClusterMetrics | null;
}
