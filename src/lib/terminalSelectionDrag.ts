...
/** Swallow left presses on the terminal after a context-menu open/dismiss (trackpad). */
let suppressTerminalPointerUntil = 0;
...
/**
 * After trackpad right-click / context-menu dismiss, the next left press on the
 * terminal must not start an xterm selection drag.
 */
export function armTerminalPointerSuppress(durationMs = 400): void {
  suppressTerminalPointerUntil = Date.now() + durationMs;
}

export function shouldSuppressTerminalPointer(): boolean {
  return Date.now() < suppressTerminalPointerUntil;
}

export function clearTerminalPointerSuppress(): void {
  suppressTerminalPointerUntil = 0;
}
...
export function getTerminalSession(sessionId: string): Terminal | null {
  return terminalBySession.get(sessionId) ?? null;
}
