...
export function formatAppError(err: unknown): string {
  ...
  if (message.includes("AI_INTERACTIVE_PASSWORD")) {
    const detail = message.replace(/^[\s\S]*?AI_INTERACTIVE_PASSWORD:\s*/, "").trim();
    return detail || i18n.t("errors:interactivePassword");
  }
  ...
  const noSuchFile = message.match(
    /^No such file(?::\s*(?:No such file(?:\s+or directory)?)?)?(?::\s*(.+))?$/i,
  );
  if (noSuchFile) {
    const path = (noSuchFile[1] || "").trim();
    if (path && !/^no such file/i.test(path)) {
      return i18n.t("errors:pathNotFound", { path });
    }
