...
export type AgentUiEvent =
  ...
  | {
      ...
      approval_id: string;
      /** Sidecar tool call id — used to show an optimistic exec card on approve. */
      call_id?: string;
      command: string;
      risk: string;
      reason: string;
  ...
  | { type: "act_nudge"; kind?: string }
  | { type: "verify_nudge"; risk?: string }
  | { type: "audit_nudge"; reason?: string; missing?: string[] }
  | { type: "assistant_retract"; reason?: string }
  | { type: "harness_sanitize"; kind?: string }
  | {
      type: "tool_timeout";
      call_id: string;
...
async function handleProtocolEvent(opts: {
  ...
  ev: StreamEvent;
  handled: Set<string>;
  /** Host tool call_ids already finished (timeout or result) — ignore late SSH returns. */
  settledHostCalls: Set<string>;
  ...
  if (ev.type === "assistant_message") {
  ...
  } else if (ev.type === "harness_sanitize") {
    onEvent({
      type: "harness_sanitize",
    ...
    const approvalEvent: Extract<AgentUiEvent, { type: "approval_needed" }> = {
      ...
      call_id: typeof p.call_id === "string" && p.call_id.trim() ? p.call_id : undefined,
    ...
    ev.type === "evidence_nudge" ||
    ev.type === "act_nudge" ||
    ev.type === "verify_nudge" ||
    ev.type === "audit_nudge"
  ) {
    if (ev.type === "evidence_nudge") {
      onEvent({
        type: "evidence_nudge",
        suppressed_chars:
          typeof p.suppressed_chars === "number" ? p.suppressed_chars : undefined,
        blocked: Boolean(p.blocked),
      });
    } else if (ev.type === "act_nudge") {
      onEvent({
        type: "act_nudge",
        kind: typeof p.kind === "string" ? p.kind : undefined,
      });
    } else if (ev.type === "audit_nudge") {
      onEvent({
        type: "audit_nudge",
        reason: typeof p.reason === "string" ? p.reason : undefined,
        missing: Array.isArray(p.missing)
          ? p.missing.map((x) => String(x))
          : undefined,
      });
    } else {
      onEvent({
    ...
    settledHostCalls.add(callId);
    onEvent({
      type: "tool_timeout",
      call_id: callId,
      timeout_seconds:
        typeof p.timeout_seconds === "number" ? p.timeout_seconds : undefined,
      error: typeof p.error === "string" ? p.error : "TOOL_TIMEOUT",
      host_may_still_be_running: Boolean(p.host_may_still_be_running),
    });
    onToolExec?.onDone?.({
      callId,
      ok: false,
    ...
    const payload =
      p.payload && typeof p.payload === "object" && !Array.isArray(p.payload)
        ...
        : {};
    onEvent({ type: "tool_result", call_id: callId, payload });
    return "continue";
    ...
    if (isHostTool) {
      ...
      // Do NOT await host exec on the stream chain — otherwise a hung sudo/
      // systemctl blocks approvals and later messages while the card stays "running".
      void (async () => {
        ...
        if (settledHostCalls.has(callId)) return;
        settledHostCalls.add(callId);
        onToolExec?.onDone?.({
          callId,
          ok: result.ok !== false,
        ...
        try {
          await postToolResultWithRetry(sidecar, {
            session_id: sessionId,
            run_id: activeRunId,
            call_id: callId,
            ok: result.ok !== false,
          ...
          // Host finished on FE but sidecar never resumed → run stays busy forever.
          onEvent({
            type: "error",
            message: `工具结果回传失败，Agent 可能卡住：${String(err)}`,
          });
        }
      })();
      return "continue";
...
export async function runAgentChat(opts: {
  ...
  securityMode?: string;
  serverId?: string;
  /** SSH host key fingerprint for TargetSessionIdentity (approval cache binding). */
  hostFingerprint?: string | null;
  /** Remote SSH username for TargetSessionIdentity. */
