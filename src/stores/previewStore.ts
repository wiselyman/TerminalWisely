...
export const usePreviewStore = create<PreviewState>((set, get) => ({
  ...
  openPreview: async (sessionId, path, sudoPassword, sizeBytes) => {
    ...
    if (!canPreviewPath(path, sizeBytes)) {
      useToastStore
        .getState()
        .pushToast(i18n.t("errors:ERR_PREVIEW_UNSUPPORTED"), false);
      return;
    }
    ...
    try {
      ...
      const rejectPreview =
        /\bERR_PREVIEW_UNSUPPORTED\b/.test(message) ||
        /\bERR_PREVIEW_TOO_LARGE\b/.test(message) ||
        message.includes("文件过大，暂不支持预览");
      if (rejectPreview) {
        set((state) => {
          const nextTabs = state.tabs.filter((tab) => tab.id !== id);
          const nextActive =
            state.activeTabId === id
              ? (previewTabsForSession(nextTabs, sessionId)[0]?.id ?? null)
              : state.activeTabId;
          return {
            tabs: nextTabs,
            ...
            sessionUi: persistSessionUi(state, sessionId, {
              activeTabId: nextActive,
              minimized: nextActive == null ? true : state.minimized,
            }),
          };
        });
