...
interface SessionState {
  ...
  reorderSavedConnections: (ids: string[]) => Promise<void>;
...
export const useSessionStore = create<SessionState>((set, get) => ({
  ...
  closeTab: async (id) => {
    ...
    try {
      ...
      // Still drop the local tab if the host session is already gone.
    }
    set((state) => {
  ...
  reorderSavedConnections: async (ids) => {
    const byId = new Map(get().savedConnections.map((item) => [item.id, item]));
    const reordered = [
      ...ids.map((id) => byId.get(id)).filter((item): item is SavedConnection => item != null),
      ...get().savedConnections.filter((item) => !ids.includes(item.id)),
    ];
    set({ savedConnections: reordered });
  ...
  connectSaved: async (savedId, password, rememberPassword, cols, rows) => {
    ...
    get().addConnectingTab({
      ...
      kind: "ssh",
      // So a second click while connecting focuses this tab instead of opening another.
      server_id: saved
    ...
    try {
      ...
      const session = mergeSessionOs(result.session, result);
      if (await discardSessionIfCancelled(pendingId, session.id)) return null;
      get().promoteConnectingTab(pendingId, session);
      await get().loadSavedConnections();
      await get().loadDeviceHistory();
      return session.id;
