...
export function AiEngineerPanel({ sessionId, serverId }: Props) {
  ...
  const platformOpen = useAiEngineerStore((s) => s.platformOpen);
  const togglePlatformView = useAiEngineerStore((s) => s.togglePlatformView);
  const openPlatformView = useAiEngineerStore((s) => s.openPlatformView);
  const setPlatformOpen = useAiEngineerStore((s) => s.setPlatformOpen);
  ...
  useEffect(() => {
    ...
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [open, togglePlatformView]);

  // CI/demo only: auto-open platform panel for screenshot capture scripts.
  useEffect(() => {
    if (!open) return;
    const mode = import.meta.env.VITE_UI_DEMO_SCREENSHOTS;
    if (mode === "1" || mode === "platform") {
      openPlatformView();
    } else if (mode === "chat") {
      setPlatformOpen(false);
    }
  }, [open, openPlatformView, setPlatformOpen]);
      ...
      >
        ...
        <header className="ai-engineer-head">
          ...
          <div className="ai-engineer-head-actions">
            <button
              type="button"
              data-testid="ai-engineer-platform-toggle"
