...
/// Reused SFTP session for browsing on the main SSH connection (avoids per-op handshakes).
pub type SharedSftpCache = Arc<Mutex<Option<Arc<Mutex<SftpSession>>>>>;

pub fn new_sftp_cache() -> SharedSftpCache {
    Arc::new(Mutex::new(None))
}

/// One entry from a remote directory listing (metadata from READDIR, no extra STAT).
#[derive(Debug, Clone)]
pub struct RemoteDirListingEntry {
    pub name: String,
    pub is_dir: bool,
    pub size_bytes: Option<u64>,
}

fn is_stale_sftp_error(err: &AppError) -> bool {
...
async fn open_shared_sftp(
    cache: &SharedSftpCache,
    handle: &Arc<Mutex<client::Handle<ClientHandler>>>,
) -> AppResult<Arc<Mutex<SftpSession>>> {
...
async fn invalidate_sftp_cache(cache: &SharedSftpCache) {
    cache.lock().await.take();
}

/// Establish the browse SFTP session early so the first directory listing skips handshake.
pub async fn warm_sftp_cache(
    cache: &SharedSftpCache,
    handle: &Arc<Mutex<client::Handle<ClientHandler>>>,
) -> AppResult<()> {
    let _ = open_shared_sftp(cache, handle).await?;
    Ok(())
}
...
async fn read_remote_directory(
    sftp: &SftpSession,
    remote_path: &str,
) -> AppResult<Vec<RemoteDirListingEntry>> {
...
pub async fn remote_file_size(
    ...
    let (_is_dir, size) = remote_path_stat(handle, remote_path).await?;
    Ok(size)
}
...
pub async fn create_remote_directory(
    ...
    let sftp = open_sftp_session(handle).await?;
    sftp.create_dir(remote_path.to_string())
        .await
        .map_err(AppError::from)?;
    Ok(())
}

pub async fn create_remote_empty_file(
    handle: &Arc<Mutex<client::Handle<ClientHandler>>>,
    remote_path: &str,
) -> AppResult<()> {
    let sftp = open_sftp_session(handle).await?;
...
pub async fn is_remote_directory(
    ...
    match remote_path_kind(handle, remote_path).await? {
        Some(true) => Ok(true),
        Some(false) => Ok(false),
        None => Err(AppError::msg(format!("No such file: {remote_path}"))),
    }
}
...
pub async fn list_remote_directory(
    ...
    let sftp = open_sftp_session(handle).await?;
    read_remote_directory(&sftp, remote_path).await
}

pub async fn list_remote_directory_cached(
    cache: &SharedSftpCache,
    handle: &Arc<Mutex<client::Handle<ClientHandler>>>,
    remote_path: &str,
) -> AppResult<Vec<RemoteDirListingEntry>> {
    for attempt in 0..2 {
        let shared = open_shared_sftp(cache, handle).await?;
        let sftp = shared.lock().await;
        match read_remote_directory(&sftp, remote_path).await {
            Ok(entries) => return Ok(entries),
            Err(err) if attempt == 0 && is_stale_sftp_error(&err) => {
                drop(sftp);
                invalidate_sftp_cache(cache).await;
                continue;
            }
            Err(err) => return Err(err),
        }
    }
    Err(AppError::msg("SFTP directory listing failed"))
}
