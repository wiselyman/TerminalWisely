...
use crate::ssh::client::{
    exec_command_capture_with_limits, ExecCaptureLimits, ExecOutputCallback,
};

use super::leases::{assert_lease_ready, consume_lease};

const MAX_STDOUT_CHARS: usize = 256 * 1024;
/// No stdout/stderr yet → fail fast (sudo hang / stalled remote).
const AI_FIRST_OUTPUT_TIMEOUT: Duration = Duration::from_secs(45);
/// After output starts, silence this long → fail (bg daemon / stuck wait).
...
pub struct AiTerminalExecRequest {
    ...
    /// Optional total wall-clock seconds (clamped). First-byte hang still uses 45s.
...
pub async fn ai_terminal_exec(
    ...
    let limits = ai_exec_limits(request.timeout_seconds);
    let (stdout, stderr, code, timed_out) = if use_sudo {
        // Never run bare interactive `sudo` on an exec channel — it hangs.
        preview_sudo::exec_remote_sudo_ai_capture(
            &handle,
            &exec_body,
        ...
        let (o, e, c, t) =
            exec_command_capture_with_limits(&handle, &command, on_output, limits).await?;
        (o, e, c as i32, t)
    };
    if timed_out {
        return Ok(AiTerminalExecResult {
            command,
            stdout: truncate(stdout, MAX_STDOUT_CHARS),
            stderr: truncate(
                if stderr.trim().is_empty() {
                    "command timed out (no output, idle after output, or exceeded time limit)".into()
                } else {
                    stderr
                },
                64 * 1024,
    ...
    // Pipelines like `sudo foo | head || …` can exit 0 while still printing
    // a password prompt — never treat that as a successful tool result.
    let combined = format!("{stdout}\n{stderr}");
    if preview_sudo::looks_like_sudo_password_needed(&combined) {
        return Err(preview_sudo::sudo_required("执行", &command));
    }
    // Only for non-sudo AI path — sudo peels use the password modal instead.
    if !use_sudo && preview_sudo::looks_like_interactive_password_prompt(&combined) {
        return Err(preview_sudo::interactive_password_required("执行", &command));
    }
