...
use crate::ssh::client::{
    exec_command, exec_command_capture_with_limits, exec_command_with_stdin,
    exec_command_with_stdin_capture_with_limits, ClientHandler, ExecCaptureLimits,
    ExecOutputCallback,
};
use crate::ssh::sftp;
...
/// Last non-empty line looks like an interactive password prompt (su / login / tty sudo).
/// AI exec channels cannot type a reply — callers should abort instead of hanging.
pub fn looks_like_interactive_password_prompt(text: &str) -> bool {
    let last = text
        .lines()
        .rev()
        .map(str::trim)
    ...
    if lower.contains("password for ") && lower.ends_with(':') {
        return true;
    }
    if lower.ends_with("password:")
        && (lower.contains("sudo")
    ...
    if lower.starts_with("enter ") && lower.contains("password") && lower.ends_with(':') {
        return true;
    }
    false
}

pub fn interactive_password_required(action: &str, path: &str) -> AppError {
    AppError::msg(format!(
        "AI_INTERACTIVE_PASSWORD: {action} `{path}` 需要交互式密码，AI 执行通道无法输入。请改用当前用户可直接执行的命令，或走可弹窗的 sudo。"
    ))
}
...
pub async fn exec_remote_sudo_capture(
    ...
    let (stdout, stderr, code, timed_out) = exec_remote_sudo_ai_capture(
        handle,
        shell_command,
        sudo_password,
        action,
    ...
    .await?;
    if timed_out {
        return Err(AppError::msg("远程命令超时（无输出或超过时限）"));
    }
    if code == 0 {
        return Ok(stdout);
    }
...
/// `limits` applies to the remote exec (first-byte / total) so interactive
/// hangs cannot block the AI tool forever.
pub async fn exec_remote_sudo_ai_capture(
...
) -> AppResult<(String, String, i32, bool)> {
    let quoted = shell_quote_remote_path(shell_command);
    if sudo_password.is_none() {
        let no_pass = format!("sudo -n sh -c {quoted}");
        let (stdout, stderr, code, timed_out) =
            exec_command_capture_with_limits(handle, &no_pass, on_output.clone(), limits).await?;
        if timed_out {
            return Ok((stdout, stderr, code as i32, true));
        }
        let combined = format!("{stdout}\n{stderr}");
        if looks_like_sudo_password_needed(&combined)
            || looks_like_interactive_password_prompt(&combined)
        {
            return Err(sudo_required(action, path_hint));
        }
        ...
        return Ok((stdout, stderr, code as i32, false));
    }
    ...
    // Same idle/first/total as non-sudo AI exec — password path must not hang on `cmd &`.
    let (bytes, stderr, code, timed_out) = exec_command_with_stdin_capture_with_limits(
        handle,
        &cmd,
        &stdin,
        512 * 1024,
    ...
    let combined = format!("{stdout}\n{stderr}");
    if looks_like_sudo_password_needed(&combined) || is_sudo_auth_failure(&AppError::msg(&combined))
    {
        return Err(sudo_required(action, path_hint));
    }
    Ok((stdout, stderr, code as i32, timed_out))
}
...
#[cfg(test)]
mod interactive_password_tests {
    use super::looks_like_interactive_password_prompt;

    #[test]
    fn detects_bare_chinese_password_prompt() {
        assert!(looks_like_interactive_password_prompt("su - u -c x\n密码："));
    }

    #[test]
    fn detects_password_for_user() {
        assert!(looks_like_interactive_password_prompt(
            "[sudo] password for alice:"
        ));
    }

    #[test]
    fn ignores_password_mention_in_logs() {
        assert!(!looks_like_interactive_password_prompt(
            "updated password policy ok"
        ));
    }
}
