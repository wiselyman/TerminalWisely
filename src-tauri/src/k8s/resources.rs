...
use super::node_metrics::enrich_node_rows;
...
impl ResourceCategory {
    pub fn from_str(s: &str) -> Option<Self> {
        match s {
            ...
            "replicationcontrollers" => Some(Self::ReplicationControllers),
            "jobs" => Some(Self::Jobs),
            ...
            "endpointslices" => Some(Self::EndpointSlices),
            ...
            "poddisruptionbudgets" | "pdb" => Some(Self::PodDisruptionBudgets),
            "priorityclasses" => Some(Self::PriorityClasses),
            "runtimeclasses" => Some(Self::RuntimeClasses),
            ...
            "validatingadmissionpolicies" => Some(Self::ValidatingAdmissionPolicies),
            "validatingadmissionpolicybindings" => Some(Self::ValidatingAdmissionPolicyBindings),
            "gatewayclasses" => Some(Self::GatewayClasses),
            "gateways" => Some(Self::Gateways),
            "httproutes" => Some(Self::HttpRoutes),
            "grpcroutes" => Some(Self::GrpcRoutes),
            "referencegrants" => Some(Self::ReferenceGrants),
    ...
    fn kubectl_kind(&self) -> &'static str {
        match self {
            ...
            Self::ValidatingAdmissionPolicies => "validatingadmissionpolicies",
            Self::ValidatingAdmissionPolicyBindings => "validatingadmissionpolicybindings",
            Self::GatewayClasses => "gatewayclasses",
            Self::Gateways => "gateways",
            Self::HttpRoutes => "httproutes",
            Self::GrpcRoutes => "grpcroutes",
            Self::ReferenceGrants => "referencegrants",
    ...
    fn row_kind(&self) -> &'static str {
        match self {
            ...
            Self::ReplicationControllers => "ReplicationController",
            Self::Jobs => "Job",
            Self::CronJobs => "CronJob",
            Self::HorizontalPodAutoscalers => "HorizontalPodAutoscaler",
            Self::Services => "Service",
            Self::Ingresses => "Ingress",
            Self::IngressClasses => "IngressClass",
            ...
            Self::PodDisruptionBudgets => "PodDisruptionBudget",
            Self::PriorityClasses => "PriorityClass",
            Self::RuntimeClasses => "RuntimeClass",
            Self::Leases => "Lease",
            Self::MutatingWebhookConfigurations => "MutatingWebhookConfiguration",
            Self::ValidatingWebhookConfigurations => "ValidatingWebhookConfiguration",
            Self::ValidatingAdmissionPolicies => "ValidatingAdmissionPolicy",
            Self::ValidatingAdmissionPolicyBindings => "ValidatingAdmissionPolicyBinding",
            Self::GatewayClasses => "GatewayClass",
            Self::Gateways => "Gateway",
            Self::HttpRoutes => "HTTPRoute",
            Self::GrpcRoutes => "GRPCRoute",
            Self::ReferenceGrants => "ReferenceGrant",
    ...
    fn is_cluster_scoped(&self) -> bool {
        matches!(
            ...
            Self::Nodes
                ...
                | Self::ValidatingAdmissionPolicies
                | Self::ValidatingAdmissionPolicyBindings
                | Self::GatewayClasses
...
pub async fn list_resources(
    ...
    if !result.ok {
        let err = result.error.unwrap_or_else(|| result.stderr.clone());
        if is_unknown_resource_error(&err) {
            return Ok(vec![]);
        }
    ...
    let items = v
        ...
        .cloned()
        .unwrap_or_default();
    let mut rows = parse_items(&items, category.row_kind())?;
    if matches!(category, ResourceCategory::Nodes) {
        enrich_node_rows(target, &mut rows, &items, sessions).await?;
    }
...
fn is_unknown_resource_error(msg: &str) -> bool {
    let lower = msg.to_lowercase();
    lower.contains("the server could not find the requested resource")
        || lower.contains("no matches for kind")
        || lower.contains("resource type not found")
        || lower.contains("unable to recognize")
        || lower.contains("doesn't have a resource type")
}

fn parse_items(items: &[Value], default_kind: &str) -> Result<Vec<K8sResourceRow>> {
    let mut rows = Vec::new();
    for item in items {
        rows.push(row_from_item(item, default_kind)?);
    }
...
fn parse_list_json(stdout: &str, default_kind: &str) -> Result<Vec<K8sResourceRow>> {
    ...
    let items = v
        ...
        .cloned()
        .unwrap_or_default();
    parse_items(&items, default_kind)
}

fn row_from_item(item: &Value, default_kind: &str) -> Result<K8sResourceRow> {
    ...
    let age = meta
        ...
        .and_then(|x| x.as_str())
        .map(format_age);
    let extra = if kind == "Node" {
        None
    } else {
        extract_extra(&item, &kind)
    };
...
pub fn rollout_restart_args(kind: &str, name: &str, namespace: &str) -> Vec<String> {
    let resource = match kind {
        "Deployment" | "deployment" => format!("deployment/{name}"),
        "StatefulSet" | "statefulset" => format!("statefulset/{name}"),
        "DaemonSet" | "daemonset" => format!("daemonset/{name}"),
        other => format!("{}/{name}", other.to_lowercase()),
    };
    let mut args = vec!["rollout".into(), "restart".into(), resource];
    if !namespace.is_empty() {
        args.push("-n".into());
        args.push(namespace.into());
    }
...
pub async fn rollout_restart(
    ...
    run_kubectl(
        target,
        &rollout_restart_args(kind, name, namespace),
        sessions,
    )
    .await
}

fn logs_resource_arg(kind: &str, name: &str) -> String {
    match kind {
        "Pod" | "pod" | "" => name.to_string(),
        "Deployment" | "deployment" => format!("deployment/{name}"),
        "StatefulSet" | "statefulset" => format!("statefulset/{name}"),
        "DaemonSet" | "daemonset" => format!("daemonset/{name}"),
        "Job" | "job" => format!("job/{name}"),
        other => format!("{}/{name}", other.to_lowercase()),
    }
}
...
mod tests {
    ...
    #[test]
    fn rollout_restart_args_for_deployment() {
        let args = rollout_restart_args("Deployment", "api", "demo");
        assert_eq!(
            args,
            vec!["rollout", "restart", "deployment/api", "-n", "demo"]
        );
    }

    #[test]
    fn logs_resource_arg_for_workload() {
        assert_eq!(logs_resource_arg("Deployment", "api"), "deployment/api");
        assert_eq!(logs_resource_arg("Pod", "api"), "api");
    }
}
