...
pub async fn start_port_forward(
    ...
    let (handle, mode) = match target.kind {
        ...
        K8sClusterKind::SshKubectl => {
            ...
            let access = crate::k8s::ssh_kubectl::SshKubectlAccess::from_target(target);
            let pf_args = shell_join_ssh(&args);
            let remote_cmd = crate::k8s::ssh_kubectl::build_ssh_login_command(
                &access,
                &format!(
                    "nohup {} {pf_args} >/tmp/tw-k8s-pf-{}.log 2>&1 & echo $!",
                    crate::k8s::ssh_kubectl::ssh_kubectl_bin(&access),
                    &id[..8]
                ),
            );
