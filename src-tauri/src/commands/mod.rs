...
use crate::types::{
    ...
    FsMoveRequest, FsPathRequest, FsRenameRequest, FsCreateRequest, FsCopyRequest,
...
pub async fn reorder_saved_connections(
    ...
    let mut connections = load_connections(&app).map_err(|e| e.to_string())?;
    let mut reordered = Vec::with_capacity(connections.len());
    for id in &ids {
        if let Some(index) = connections.iter().position(|c| &c.id == id) {
            reordered.push(connections.remove(index));
        }
    }
    reordered.extend(connections);
    store_connections(&app, &reordered).map_err(|e| e.to_string())?;
    Ok(())
}
...
#[tauri::command]
pub async fn ai_chat_cache_remote_media(
    app: tauri::AppHandle,
    url: String,
) -> Result<crate::ai_engineer::CachedRemoteMedia, String> {
    crate::ai_engineer::cache_remote_media(&app, &url)
        .await
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub fn ai_chat_resolve_media(
    app: tauri::AppHandle,
    media_id: String,
) -> Result<crate::ai_engineer::CachedRemoteMedia, String> {
    crate::ai_engineer::resolve_media_id(&app, &media_id).map_err(|e| e.to_string())
}

#[tauri::command]
pub fn ai_chat_history_load(
    app: tauri::AppHandle,
) -> Result<crate::ai_engineer::ChatHistorySnapshot, String> {
    // Index only (no message bodies) — full bodies via ai_chat_history_load_scope.
    crate::ai_engineer::load_chat_history_index(&app).map_err(|e| e.to_string())
}

#[tauri::command]
pub fn ai_chat_history_load_scope(
    app: tauri::AppHandle,
    scope: String,
) -> Result<Option<crate::ai_engineer::ScopeThreadBundle>, String> {
    crate::ai_engineer::load_chat_scope(&app, &scope).map_err(|e| e.to_string())
}

#[tauri::command]
pub fn ai_chat_history_save_scope(
    app: tauri::AppHandle,
    scope: String,
    bundle: crate::ai_engineer::ScopeThreadBundle,
) -> Result<(), String> {
    crate::ai_engineer::save_scope_bundle(&app, scope, bundle).map_err(|e| e.to_string())
}

#[tauri::command]
pub fn ai_chat_history_save_all(
    app: tauri::AppHandle,
    by_scope: std::collections::HashMap<String, crate::ai_engineer::ScopeThreadBundle>,
) -> Result<(), String> {
    crate::ai_engineer::save_all_scopes(&app, by_scope).map_err(|e| e.to_string())
}

#[tauri::command]
pub fn ai_chat_history_import_localstorage(
    app: tauri::AppHandle,
    by_scope: std::collections::HashMap<String, crate::ai_engineer::ScopeThreadBundle>,
    force: Option<bool>,
) -> Result<crate::ai_engineer::ChatHistorySnapshot, String> {
    crate::ai_engineer::import_localstorage_snapshot(&app, by_scope, force.unwrap_or(false))
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub fn ai_chat_history_mark_migrated(app: tauri::AppHandle) -> Result<(), String> {
    crate::ai_engineer::mark_migrated(&app).map_err(|e| e.to_string())
}
...
/// Reveal a local file in the OS file manager, or open a directory.
#[tauri::command]
...
#[tauri::command]
pub fn k8s_update_ssh_binding_session(id: String, session_id: String) -> Result<(), String> {
    crate::k8s::update_ssh_binding_session(&id, &session_id).map_err(|e| e.to_string())
}
...
pub async fn k8s_cordon_node(
    ...
    crate::k8s::cordon_node(&target, &name, &sessions)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn k8s_uncordon_node(
    ...
    crate::k8s::uncordon_node(&target, &name, &sessions)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn k8s_drain_node(
    ...
    crate::k8s::drain_node(&target, &name, &sessions)
        .await
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub fn k8s_node_shell_command(
    target: crate::k8s::K8sClusterTarget,
    node: String,
) -> Result<String, String> {
    Ok(crate::k8s::node_shell_command(&target, &node))
}

#[tauri::command]
pub fn k8s_node_shell_start(
    app: AppHandle,
    target: crate::k8s::K8sClusterTarget,
    node: String,
    cols: u16,
    ...
    crate::k8s::start_node_shell(app, &target, &node, cols, rows).map_err(|e| e.to_string())
}
...
pub async fn k8s_pod_logs(
    ...
    crate::k8s::resource_logs(
        &target,
        kind.as_deref().unwrap_or("Pod"),
        &namespace,
        &pod,
        container.as_deref(),
...
#[tauri::command]
pub async fn k8s_list_crd_catalog(
    sessions: State<'_, SessionManager>,
    target: crate::k8s::K8sClusterTarget,
) -> Result<Vec<crate::k8s::K8sCrdCatalogEntry>, String> {
    crate::k8s::list_crd_catalog(&target, &sessions)
        .await
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn k8s_list_applications(
    sessions: State<'_, SessionManager>,
    target: crate::k8s::K8sClusterTarget,
    namespace: Option<String>,
) -> Result<Vec<crate::k8s::K8sResourceRow>, String> {
    crate::k8s::list_applications(&target, namespace.as_deref(), &sessions)
        .await
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn k8s_helm_list_charts(
    sessions: State<'_, SessionManager>,
    target: crate::k8s::K8sClusterTarget,
) -> Result<Vec<crate::k8s::HelmChartRow>, String> {
    crate::k8s::list_helm_charts(&target, &sessions)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn k8s_helm_list_repos(
    sessions: State<'_, SessionManager>,
    target: crate::k8s::K8sClusterTarget,
) -> Result<Vec<crate::k8s::HelmRepoRow>, String> {
    crate::k8s::list_helm_repos(&target, &sessions)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn k8s_helm_add_repo(
    ...
    crate::k8s::add_helm_repo(&target, &name, &url, &sessions)
        .await
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn k8s_helm_chart_values(
    sessions: State<'_, SessionManager>,
    target: crate::k8s::K8sClusterTarget,
    chart: String,
) -> Result<String, String> {
    crate::k8s::helm_chart_values(&target, &chart, &sessions)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn k8s_helm_rollback(
    ...
    crate::k8s::helm_rollback(&target, &release, &namespace, &sessions)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn k8s_helm_uninstall(
    ...
    crate::k8s::helm_uninstall(&target, &release, &namespace, &sessions)
        .await
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn k8s_rollout_restart(
    sessions: State<'_, SessionManager>,
    target: crate::k8s::K8sClusterTarget,
    kind: String,
    namespace: String,
    ...
    crate::k8s::rollout_restart(&target, &kind, &namespace, &name, &sessions)
        .await
        .map_err(|e| e.to_string())
}
...
#[tauri::command]
pub fn k8s_kubectl_cluster_shell_start(
    app: tauri::AppHandle,
    target: crate::k8s::K8sClusterTarget,
    cols: Option<u16>,
    rows: Option<u16>,
) -> Result<crate::k8s::K8sPodShellInfo, String> {
    crate::k8s::start_kubectl_cluster_shell(
        app,
        &target,
        cols.unwrap_or(120),
        rows.unwrap_or(32),
