...
impl SessionManager {
    ...
    /// Live SSH tab for a saved host key (`user@host:port`), if any.
    pub async fn find_ssh_session_by_server_id(&self, server_id: &str) -> Option<String> {
        ...
        self.sessions
            ...
            .find_map(|handle| {
                ...
                if info.kind == SessionKind::Ssh && info.server_id.as_deref() == Some(needle) {
                    Some(info.id)
                } else {
                    None
                }
    ...
    pub async fn rename_path(
        ...
        // Host tree reloads via SFTP; do not inject `ls` into the interactive PTY.
        Ok(())
    }
    ...
    pub async fn create_path(
        ...
        if kind != "file" && kind != "directory" {
            return Err(AppError::msg("kind 必须是 file 或 directory"));
        }
        let ssh = self.ssh_snapshot(session_id).await?;
        let parent = ssh.resolve_remote_path(parent_dir).await?;
        if !sftp::is_remote_directory(&ssh.handle(), &parent).await? {
            return Err(AppError::msg("父路径必须是目录"));
        }
        let new_path = remote_join_path(&parent, name.trim());
        let result = if kind == "directory" {
            sftp::create_remote_directory(&ssh.handle(), &new_path).await
        } else {
            sftp::create_remote_empty_file(&ssh.handle(), &new_path).await
        };
        if let Err(err) = result {
            if preview_sudo::is_permission_denied(&err) {
                let quoted = shell_quote_remote_path(&new_path);
                let cmd = if kind == "directory" {
                    format!("mkdir -- {quoted}")
                } else {
                    format!("touch -- {quoted}")
                };
    ...
    pub async fn copy_path(
        ...
        if is_same_or_descendant_path(&resolved, &dest) {
            return Err(AppError::msg("不能复制到自身或其子目录"));
        }
        ...
        let new_path = remote_join_path(&dest, file_name);
        if new_path == resolved {
            return Err(AppError::msg("目标与源相同"));
        }
        let quoted_src = shell_quote_remote_path(&resolved);
        let quoted_dest = shell_quote_remote_path(&new_path);
        let cmd = format!("cp -a -- {quoted_src} {quoted_dest}");
    ...
    pub async fn duplicate_path(
        ...
        let resolved = ssh.resolve_remote_path(path).await?;
        let parent = remote_parent_path(&resolved)?;
        let base_name = std::path::Path::new(&resolved)
            .file_name()
            .and_then(|n| n.to_str())
            .ok_or_else(|| AppError::msg("无效的文件名"))?;
        let candidate = unique_copy_name(base_name);
        let dest = remote_join_path(&parent, &candidate);
        let quoted_src = shell_quote_remote_path(&resolved);
        let quoted_dest = shell_quote_remote_path(&dest);
        let cmd = format!("cp -a -- {quoted_src} {quoted_dest}");
        if let Err(err) = crate::ssh::client::exec_command(&ssh.handle(), &cmd).await {
            if preview_sudo::is_permission_denied(&err) {
                preview_sudo::exec_remote_sudo(
...
/// True when `candidate` is the same as `source` or nested under it.
pub fn is_same_or_descendant_path(source: &str, candidate: &str) -> bool {
    let src = normalize_remote_path(source);
    let cand = normalize_remote_path(candidate);
    if src.is_empty() || cand.is_empty() {
        return false;
    }
    if cand == src {
        return true;
...
fn unique_copy_name(base_name: &str) -> String {
    ...
    let (stem, ext) = match base_name.rsplit_once('.') {
        Some((s, e)) if !s.is_empty() && !e.is_empty() && !s.contains('/') => {
            (s, Some(e))
        }
        _ => (base_name, None),
    };
    let with_ext = |label: &str| -> String {
        match ext {
            Some(e) => format!("{label}.{e}"),
            None => label.to_string(),
        }
    };
    // Backend always picks "… copy"; collision handled by cp failing (rare).
    with_ext(&format!("{stem} copy"))
}
...
mod fs_path_tests {
    ...
    fn rejects_bad_names() {
        ...
        assert!(validate_fs_name("ok.txt").is_ok());
    }

    #[test]
    fn descendant_detection() {
        assert!(is_same_or_descendant_path("/a/b", "/a/b"));
        assert!(is_same_or_descendant_path("/a/b", "/a/b/c"));
        assert!(!is_same_or_descendant_path("/a/b", "/a/bc"));
        assert!(!is_same_or_descendant_path("/a/b", "/a"));
    }
