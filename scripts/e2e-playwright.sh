...
# Free the port even if a stray preview/dev server was left running.
if command -v lsof >/dev/null 2>&1; then
  lsof -tiTCP:"${PREVIEW_PORT}" -sTCP:LISTEN 2>/dev/null | xargs kill -9 2>/dev/null || true
fi
...
for _ in $(seq 1 30); do
  if ! kill -0 "$PREVIEW_PID" 2>/dev/null; then
    ...
    tail -40 /tmp/e2e-preview.log >&2 || true
    exit 1
  fi
  if curl -sf "${E2E_BASE_URL}/" >/dev/null; then break; fi
  sleep 0.5
done
if ! kill -0 "$PREVIEW_PID" 2>/dev/null || ! curl -sf "${E2E_BASE_URL}/" >/dev/null; then
  echo "Vite preview failed" >&2
  tail -40 /tmp/e2e-preview.log >&2 || true
  exit 1
fi
if grep -q "Port ${PREVIEW_PORT} is already in use" /tmp/e2e-preview.log 2>/dev/null; then
  echo "Vite preview did not own :${PREVIEW_PORT} (stale process)" >&2
  tail -40 /tmp/e2e-preview.log >&2 || true
  exit 1
fi
